    ...
    public static class JsonParser
    {
        ...
        private static object ParseUnkownValue(string value)
        {
            ...
            else if (value[0] == '[' && value[value.Length - 1] == ']')
            {
                ...
                Type listType = typeof(List<>).MakeGenericType(output.Any(o => o != null && o.GetType() != output.First().GetType()) ? typeof(object) : output.First().GetType());
            ...
            else if (value[0] == '{' && value[value.Length - 1] == '}')
            {
                ...
                {
                    string key = (string)ParseUnkownValue(dict[i]);
                    if (key == null) return null;
                    output.Add(key, ParseUnkownValue(dict[i + 1]));
                }
